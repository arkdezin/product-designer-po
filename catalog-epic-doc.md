...
A[Login into Admin Portal]
A -->|Opens Side navbar and navigates to| B[Catalog Maintenance]
B -->|Lands on| C[Catalog Maintenance Dashboard]
C -->|Default dashboard view| D[Dashboard with stats, catalog products, filters, views, column options]

%% LEFT: Current Catalog Products
D -->|Default view| E[Current Catalog Products - Live]
E --> F[Add or suggest modification on products]
F --> G["`Suggest modification screen
Raw data + AI enhanced + Live catalog data`"]
G --> H["Select any field(s) to enable modification, add a reason for modification"]
H --> I["`Submit for approval -
Second review process starts`"]

%% FILTER PRODUCTS
D -->|Action| J[Filter catalog products]
J --> J1["`Filters
...
D -->|Consolidated view| K[Create view or workspace with filters]
K --> K1["`Filters
...
- Confidence threshold`"]
  K1 --> K2[Name the view]
  K2 -->|Stats/kpi will be updated based on the view| K3[View or Workspace Created]

%% COLUMN OPTIONS
D -->|Action| L["`Column options
(Modify columns as per user requirement)`"]
L --> L1[Show or hide columns in table]

%% RIGHT: CATALOG QUEUE
D -->|Navigation| M["Catalog Queue (pending review product)"]
M --> N["`View all pending review products
(Not live in catalog)`"]
N --> O["`Filter by
...
O --> P[Assign products to users]
O -->|filtered route| Q[Open product review page]

P --> R["`Review screen
Raw data + AI enhanced + final data`"]
Q --> R
N -->|Direct Route| Q

%% REVIEW ACTIONS
R --> S[Default raw data selected and updated on the final data]
R -->|Actions| T["`-Save as draft
...
- Back to queue`"]
  S --> U
  T --> U[Before approval actions]
  U --> V[Mark confident or non confident]
  U --> W[Mark need clarification]
  U --> X["Mark product for revisit
  (1/2/4 or 8 weeks etc.)"]
```
...
```mermaid
graph TD
A[Logs into Admin Portal] -->|Direct open no other module access| B[Opens Catalog Queue Page]
B --> C["View products with New Product status and Supplemental Adhoc Products (Not live on catalog)"]
C --> D["`Filter Products
- UPC
- Brand
- Edited by
...
D --> E[Manager assigns products to team members]
E --> F[Team member opens assigned product review page]

F --> G["View screen with
Raw Data + AI-Enhanced + Final Data"]

G --> H[Default raw data selected and updated on the final data]
G -->|"Actions (CTA)"| I["`
- Save as draft
- Send for approval
- Need Clarification
...
H --> J[Before sending for approval user can do these actions]
I --> J

J --> K[Verify & Validate Data]
J --> L[Mark confident / non-confident]
J --> M[Mark need clarification]
...
- organic,
- PLU
**via AI suggestion or product page link**`"]
K --> K3["`Select dropdowns based on **dictionary**`"]
```
